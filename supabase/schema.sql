...
values
('colombia','Colombia','phase1','CO',1,'Sep 2026','Colombia begins The 210 Project with colour, movement and arrival.',0,'Medellin',array['First solo travel rhythm','Coffee region','Caribbean coast'],'sage',21,420,82,21,1180),
('peru','Peru','phase1','PE',2,'Sep / Oct 2026','Peru is the endurance chapter. Altitude, history and trekking turn the journey into a test of rhythm, patience and resilience.',0,'Cusco',array['Machu Picchu sunrise','Cusco altitude lesson','Paracas coastline'],'copper',24,690,120,24,2040),
('argentina','Argentina','phase1','AR',3,'Oct / Nov 2026','Argentina blends city energy with wilderness: a chapter about scale, long journeys, food, culture and the symbolic edge of the continent.',0,'Buenos Aires',array['Buenos Aires evenings','Patagonia hikes','Ushuaia'],'terracotta',24,620,110,21,3600),
...
('brazil','rio','Rio','Dec 2026','Colour, rhythm and the closing note of South America.','Rio closes the first act of The 210 Project. The archive here is less about motion and more about presence.',array['Sunset','Beach walks','Closing reflection'],array['joy','reflection','coast'],'https://images.unsplash.com/photo-1483729558449-99ef09a8c325?auto=format&fit=crop&w=1200&q=80',1)
on conflict (country_id, slug) do nothing;

insert into public.site_settings (id,current_country_id,current_location_slug)
values ('main','peru','cusco')
on conflict (id) do update set current_country_id=excluded.current_country_id,current_location_slug=excluded.current_location_slug;

-- Supabase Storage bucket for phone uploads.
insert into storage.buckets (id, name, public)
values ('trip-media', 'trip-media', true)
on conflict (id) do update set public = true;

drop policy if exists "public read trip media" on storage.objects;
create policy "public read trip media" on storage.objects for select using (bucket_id = 'trip-media');

drop policy if exists "authenticated upload trip media" on storage.objects;
create policy "authenticated upload trip media" on storage.objects for insert to authenticated with check (bucket_id = 'trip-media');

drop policy if exists "authenticated update trip media" on storage.objects;
create policy "authenticated update trip media" on storage.objects for update to authenticated using (bucket_id = 'trip-media') with check (bucket_id = 'trip-media');

drop policy if exists "authenticated delete trip media" on storage.objects;
create policy "authenticated delete trip media" on storage.objects for delete to authenticated using (bucket_id = 'trip-media');
